...
$\xvar{nUncon}$ $\leftarrow$ $\mysum(\xvar{M}(\xvar{iNonSat}))$ \tcc*{\# of mandatory comps not in saturated subgraph}
  ...
  \For(\tcc*[f]{add remaining edges in arbitrary order\label{alg:app1:saturated-l6}}){ $\xvar{j}$ $\leftarrow$ $1$ \KwTo $\mysum(\xvar{V})$}{

  {$\xvar{k}$} $\leftarrow$ $\find(\xvar{V},1)$ \tcc*{find first nonzero entry}

  {$\xvar{LR}$} $\leftarrow$ $\xvar{cVf}(\xvar{k})-\xvar{V}(\xvar{k})$ % \tcc*{port}

  {$\xvar{V}(\xvar{k})$} $\leftarrow$ $\xvar{V}(\xvar{k})-1$ \tcc*{remove port}
...
\uElseIf(\tcc*[f]{no mandatory comps are in saturated subgraph\label{alg:app1:saturated-l14}}){$\xvar{nUncon} = \mysum(M)$}{
